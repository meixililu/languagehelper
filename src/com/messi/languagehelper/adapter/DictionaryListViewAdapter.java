...
public class DictionaryListViewAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			holder.record_question_cover = (FrameLayout) convertView
					.findViewById(R.id.record_question_cover);
			holder.record_answer_cover = (FrameLayout) convertView
					.findViewById(R.id.record_answer_cover);
			holder.record_question = (TextView) convertView
					.findViewById(R.id.record_question);
			holder.record_answer = (TextView) convertView
		...
		AnimationDrawable animationDrawable = (AnimationDrawable) holder.voice_play
				.getBackground();
		MyOnClickListener mMyOnClickListener = new MyOnClickListener(mBean,
				animationDrawable, holder.voice_play,
		...
		holder.record_question_cover
				.setOnClickListener(mQuestionOnClickListener);
	...
	private String getMeans(List<Means> mMeans, boolean isEnglishResult)
			throws Exception {
		StringBuilder sb = new StringBuilder();
	...
	private void sendToWechat(final Dictionary mBean) {
		...
		mPopDialog.setListener(new PopViewItemOnclickListener() {
			...
			public void onSecondClick(View v) {
				...
				StatService.onEvent(context, "1.8_to_share_image_btn",
						"去图片分享页面按钮", 1);
			}
	...
	private void updateCollectedStatus(Dictionary mBean) {
		if (mBean.getIscollected().equals("0")) {
			...
			showToast(context.getResources()
					.getString(R.string.favorite_cancle));
		}
	...
	public class MyOnClickListener implements OnClickListener {
		...
		public void onClick(final View v) {
			...
			// TextUtils.isEmpty(mBean.getQuestionVoiceId())){
			...
			// }
			// @Override
			// public void onBufferProgress(int arg0, int arg1, int arg2, String
			// arg3) {
			// }
			// @Override
